...
if re_create or not os.path.exists(predict_model_rawinput):
    ...
    raw_audio_input = Input(shape=[None,1],name='raw_audio')

    def extra_spec(raw_audio_input):
        from Scripts.Models.DataParsers.Features.mel import raw_audio2log_mel_spec_op
        from config import tf_mel_fea
        SAMPLE_RATE = 16000
...
converter = tf.lite.TFLiteConverter.from_keras_model_file(predict_model_rawinput,input_shapes = {"raw_audio" : [None,8000]})
